...
|---|---|---|
| `kividb` | `spec.image` | The database itself. `workingDir: /data`, `--configfile /etc/kividb/kividb.conf`. |
...
1. Resolve every Secret referenced by `spec.auth` (requirepass + per-user
   passwords) and render the ACL file content in memory
   (`renderACLFile`), hashing passwords with SHA-256 exactly like kividb's
   own `ACL SETUSER ... >password` would.
2. Create/update the `<cluster>-auth` Secret (ACL file) and
   `<cluster>-config` ConfigMap (kividb.conf, rendered from
   `spec.kividbConfig` plus the operator-owned `port`/`aclfile`
   directives — `replicaof` is deliberately never written here; topology
   is runtime-managed, not config-file-managed).
3. Create/update the headless, master and replica Services.
4. Create/update the StatefulSet (pod template only; `Selector`,
   ...
   changes to those fields on an existing StatefulSet).
5. Create/update (or delete, if `spec.backup.enabled` is false) the backup
   CronJob.
6. List the cluster's pods and run **role reconciliation** (next section).
7. Write `status` (phase, masterPod, per-pod role/ready/offset, conditions).
8. Requeue after 10 seconds — this periodic tick, plus a Pod-watch that
...
(`<cluster>-backup`) — the operator does not implement its own cron
scheduler. The CronJob's pod runs the **same agent binary** in a different
mode: `agent backup-trigger --url http://<cluster>-master.<ns>.svc:8081/backup`.
...
pod is currently master, because:

1. The URL targets the **master Service**, which always resolves to the
   current master pod regardless of any failovers that happened since the
   CronJob was created.
2. The master pod's own agent sidecar (which already has the data volume
   mounted and the S3 env vars injected by the controller) does the real
   work: `BGSAVE`, poll `LASTSAVE` until it advances, tar+gzip
...
which is manual by design (there is no "restore" verb on the CRD).
...
- No mutating/validating admission webhook — `spec` validation is
  ...
  weaker cross-field validation (e.g. `spec.backup.schedule` being
  required only when `spec.backup.enabled` is checked at reconcile time,
  not at `kubectl apply` time).
